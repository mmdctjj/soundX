 ...
 * 封面 / 头像的分级加载档位表。
 *
 * 背景：后端 `GET /image/optimize` 按 `sha256(src|w|q|fmt)` 落盘缓存
 ...
 *
 * 档位语义 = **目标设备像素宽度**（不是 CSS 尺寸）。
 * desktop 现有调用点（如 TrackList 的 `{ width: 60 }`）本来就是按 2x 设备像素写的，
 * 保持这个语义可以零改动对齐。新增调用点按「显示尺寸 × 2」估算即可；
 ...
 * 返回 true 时不读网络状态，一律走 /image/optimize。
 */
